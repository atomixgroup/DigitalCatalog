...
public class FileManagerActivity extends AppCompatActivity implements AdapterFileManager.ListItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(intent.hasExtra("formats")){
            String extraFormat=intent.getExtras().getString("formats");
            formats=extraFormat.split(",");
        }
        ...
        if(files!=null) {
            for (File singleFile : files) {
                ...
                if (singleFile.isDirectory()) {
                    ...
                    if(formats.length>0){
                        String fileExt=App.getFileExtension(temp.getFileName());
                        boolean flag=false;
                        for (String form:formats) {
                               if(form.equals(fileExt)){
                                   flag=true;
                               }
                        }
                        if(!flag) continue;
                        ...
                        if(!App.isAwaibleFormat(temp.getFileName())){
                            continue;
                        }
                    }
